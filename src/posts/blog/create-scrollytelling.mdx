---
title: "Creating Interactive Scrollytelling in Astro"
description: "A comprehensive guide on creating dynamic, interactive scrollytelling pages using Astro, MDX, and D3.js, inspired by the 'Beyond the Screen' project."
...
The journey begins with the content. We use an MDX file (`src/posts/scrolly/media-effects-election.mdx`) as the entry point. The beauty of MDX is that it allows us to interweave standard Markdown with custom components.
...
To preserve the readability of the MDX file, all the heavy lifting for the charts is extracted into a dedicated configuration file (`src/scrolly/data/media-effects-election.ts`).
